...
export function useBrevoAuth() {
  ...
  const setStateField = useCallback(<K extends keyof UseBrevoAuthState>(
    field: K,
    value: UseBrevoAuthState[K]
  ) => {
  ...
  const sendOTP = useCallback(async (data: {
    ...
    try {
      ...
      if (result.success) {
        ...
        setStateField('success', `Code envoyé par ${method === 'phone' ? 'WhatsApp/SMS' : 'email'} !`);
        return true;
      } else {
   ...
   */
  const submitName = useCallback(async (fullName: string): Promise<boolean> => {
   ...
   */
  const selectRole = useCallback(async (role: 'locataire' | 'proprietaire' | 'agence') => {
    setStateField('loading', true);
    clearError();
    clearSuccess();
