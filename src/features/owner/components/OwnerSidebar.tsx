...
export default function OwnerSidebar({ isOpen, onClose }: OwnerSidebarProps) {
  ...
  const { ownerItems, bottomItems } = useNavigationItems();

  const isActive = (href: string) => {
    if (href === '/dashboard') {
      return currentPath === '/dashboard' || currentPath === '/dashboard/proprietaire';
    }
      ...
      >
        ...
        <div className="p-4 border-t border-neutral-100">
          <div className="bg-primary-50 rounded-xl p-4">
            <p className="text-sm font-medium text-primary-700 mb-1">Gérer vos biens</p>
            <p className="text-xs text-primary-600 mb-3">Ajoutez une nouvelle propriété</p>
            <Link
