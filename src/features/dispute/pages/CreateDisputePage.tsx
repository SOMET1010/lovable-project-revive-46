...
const CATEGORIES = [
  { value: 'payment', label: 'Paiement', icon: Banknote, description: 'Loyer impayé, charges contestées' },
  { value: 'deposit', label: 'Caution', icon: Banknote, description: 'Retenue de caution, remboursement' },
  { value: 'maintenance', label: 'Maintenance', icon: Wrench, description: 'Travaux non effectués, réparations' },
  { value: 'noise', label: 'Nuisances', icon: Volume2, description: 'Bruit, troubles de voisinage' },
  { value: 'damages', label: 'Dégâts', icon: FileWarning, description: 'Dégradations, état des lieux' },
  { value: 'lease_violation', label: 'Violation bail', icon: Home, description: 'Non-respect du contrat' },
...
export default function CreateDisputePage() {
  ...
  const [contracts, setContracts] = useState<Array<{ id: string; property_title: string; other_party_name: string; other_party_id: string }>>([]);
  ...
  const loadContracts = async () => {
    ...
    if (!error && data) {
      const formattedContracts = await Promise.all(data.map(async (contract) => {
        const otherPartyId = contract.owner_id === user.id ? contract.tenant_id : contract.owner_id;
        
        const { data: profile } = await supabase
          .rpc('get_public_profile', { profile_user_id: otherPartyId });
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.category || !formData.subject || !formData.description || !formData.respondent_id) {
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div className={`p-3 rounded-xl ${
                        formData.category === cat.value ? 'bg-[#F16522] text-white' : 'bg-[#EFEBE9] text-[#2C1810]'
                      }`}>
              ...
              <div className="flex items-start gap-3 p-4 bg-amber-50 rounded-xl border border-amber-200">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-amber-700 mt-1">
                    En soumettant ce litige, vous acceptez que notre équipe de médiation contacte les deux parties pour tenter de trouver une solution amiable.
                  </p>
                </div>
              </div>
