          ...
          {weekDays.map(day => {
            ...
            return (
              <div 
                ...
                `}
              >
                <span className={`text-[10px] uppercase font-medium ${isDayToday ? '' : 'text-muted-foreground'}`}>
                  ...
                  {dayMissions.length > 0 && (
                    <>
                      {dayMissions.length <= 3 ? (
                        ...
                        <>
                          <div className={`w-1.5 h-1.5 rounded-full ${urgencyColors[urgency || 'low']}`} />
                          <span className={`text-[8px] font-medium ${isDayToday ? '' : 'text-muted-foreground'}`}>
                            +{dayMissions.length}
                          </span>
                        </>
                      )}
            ...
            <div className="space-y-2">
              {todaysMissions.map(mission => {
                ...
                return (
                  ...
                  >
                    <div className={`w-1 h-8 rounded-full ${urgencyColors[mission.urgency] || 'bg-muted-foreground'}`} />
              ...
              {totalTodayMissions > 3 && (
                ...
                >
                  +{totalTodayMissions - 3} autre{totalTodayMissions - 3 > 1 ? 's' : ''} mission{totalTodayMissions - 3 > 1 ? 's' : ''}
                </Link>
              )}
            </div>
          )}
