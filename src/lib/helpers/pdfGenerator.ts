...
export const generateLeasePDF = (leaseData: LeaseData): jsPDF => {
  ...
  const addLine = (text: string | string[], options: {
    bold?: boolean,
    size?: number,
    indent?: number,
    spacing?: number,
  ...
  addLine('CONTRAT DE BAIL À USAGE D\'HABITATION', { bold: true, size: 16, align: 'center', spacing: 8 });
  addLine('(Modèle Conforme au Code Civil Ivoirien)', { size: 11, align: 'center', spacing: 8 });

  addSeparator();

  addLine(`Référence du Contrat: ${leaseData.contractNumber}`, { bold: true, size: 10, spacing: 6 });
  addLine(`Date d'établissement: ${new Date().toLocaleDateString('fr-FR')}`, { size: 10, spacing: 10 });
  ...
  addLine('Le BAILLEUR donne à bail au LOCATAIRE qui accepte, le local à usage d\'habitation désigné ci-après, que ce dernier déclare bien connaître pour l\'avoir visité avant la signature du présent contrat.', { size: 10, spacing: 10 });

  addLine('ARTICLE 2 : DÉSIGNATION DU LOCAL LOUÉ', { bold: true, size: 11, spacing: 8 });
  addLine('Le bien immobilier objet du présent bail est situé à l\'adresse suivante :', { size: 10, spacing: 7 });
  addLine([
  ...
  addLine('Le LOCATAIRE déclare avoir visité les lieux et les accepter en l\'état.', { size: 10, spacing: 10 });

  addLine('ARTICLE 3 : DURÉE DU BAIL', { bold: true, size: 11, spacing: 8 });
  addLine('Le présent bail est consenti et accepté pour une durée déterminée de :', { size: 10, spacing: 7 });
  addLine([
  ...
  addLine(`Le loyer mensuel est fixé à la somme de: ${leaseData.monthlyRent.toLocaleString('fr-FR')} FRANCS CFA`, { size: 10, spacing: 6 });
  addLine(`Ce loyer est payable d'avance, le ${leaseData.paymentDay} de chaque mois.`, { size: 10, spacing: 8 });

  addLine('4.2 - CHARGES', { bold: true, size: 10, spacing: 7 });
  addLine(`Les charges mensuelles s'élèvent à: ${leaseData.chargesAmount.toLocaleString('fr-FR')} FRANCS CFA`, { size: 10, spacing: 8 });

  addLine('4.3 - DÉPÔT DE GARANTIE', { bold: true, size: 10, spacing: 7 });
  addLine(`À la signature du présent bail, le LOCATAIRE verse au BAILLEUR un dépôt de garantie d'un montant de: ${leaseData.depositAmount.toLocaleString('fr-FR')} FRANCS CFA`, { size: 10, spacing: 6 });
  addLine('Ce dépôt sera restitué au LOCATAIRE dans un délai maximum de deux (2) mois après son départ, déduction faite, le cas échéant, des sommes restant dues et du coût des réparations locatives mises à sa charge.', { size: 10, spacing: 10 });

  addPageIfNeeded(80);

  addLine('ARTICLE 5 : ÉTAT DES LIEUX', { bold: true, size: 11, spacing: 8 });
  addLine('Un état des lieux contradictoire sera établi lors de la remise des clés au LOCATAIRE et lors de leur restitution au BAILLEUR. Cet état des lieux sera annexé au présent contrat et signé par les deux parties.', { size: 10, spacing: 10 });

  addLine('ARTICLE 6 : DESTINATION DES LIEUX', { bold: true, size: 11, spacing: 8 });
  addLine('Les lieux loués ne pourront servir qu\'à l\'usage d\'habitation du LOCATAIRE et de sa famille. Toute autre utilisation, notamment commerciale ou professionnelle, est strictement interdite sans l\'accord écrit préalable du BAILLEUR.', { size: 10, spacing: 10 });
  ...
  addLine('Le LOCATAIRE doit obligatoirement souscrire une assurance contre les risques locatifs et en justifier au BAILLEUR dès la remise des clés. À défaut d\'assurance, le BAILLEUR pourra résilier le bail de plein droit après mise en demeure restée sans effet pendant un délai d\'un mois.', { size: 10, spacing: 10 });

  addPageIfNeeded(60);

  addLine('ARTICLE 10 : RÉSILIATION DU BAIL', { bold: true, size: 11, spacing: 8 });
  addLine('Le LOCATAIRE peut résilier le bail à tout moment en respectant un préavis de trois (3) mois. Le BAILLEUR peut donner congé au LOCATAIRE en respectant un préavis de six (6) mois dans les conditions prévues par la loi.', { size: 10, spacing: 10 });
  ...
  addLine('Le présent contrat est établi en deux (2) exemplaires originaux, dont un pour chaque partie.', { size: 10, spacing: 7 });
  addLine('Les parties reconnaissent la valeur juridique de la signature électronique certifiée par ANSUT et horodatée par CryptoNeo, conformément à la législation ivoirienne sur les transactions électroniques.', { size: 9, spacing: 10 });
